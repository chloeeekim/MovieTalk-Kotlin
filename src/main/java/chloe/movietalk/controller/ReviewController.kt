...
interface ReviewController {
    ...
    @ApiResponses(
        value = [ApiResponse(
            ...
            responseCode = "404",
            description = "해당 ID의 리뷰가 존재하지 않습니다.",
            content = [Content(schema = Schema(implementation = ErrorResponse::class))]
        )]
    )
    fun updateReview(
    ...
    @ApiResponses(
        value = [ApiResponse(
            responseCode = "200",
            description = "성공",
            content = [Content(array = ArraySchema(schema = Schema(implementation = ReviewByMovieResponse::class)))]
        ), ApiResponse(
            responseCode = "404",
            description = "해당 ID의 영화가 존재하지 않습니다.",
            content = [Content(schema = Schema(implementation = ErrorResponse::class))]
    ...
    @ApiResponses(
        value = [ApiResponse(
            ...
            responseCode = "400",
            description = "이미 좋아요 표시된 리뷰입니다.",
            content = [Content(schema = Schema(implementation = ErrorResponse::class))]
        ), ApiResponse(
            responseCode = "404",
            description = "해당 ID의 사용자나 리뷰가 존재하지 않습니다.",
            content = [Content(schema = Schema(implementation = ErrorResponse::class))]
        )]
    )
    fun likeReview(
    ...
    @ApiResponses(
        value = [ApiResponse(
            ...
            responseCode = "404",
            description = "좋아요 기록이 없거나, 해당 ID의 사용자나 리뷰가 존재하지 않습니다.",
            content = [Content(schema = Schema(implementation = ErrorResponse::class))]
        )]
    )
    fun unlikeReview(
