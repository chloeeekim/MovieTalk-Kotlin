...
class ReviewControllerImpl : ReviewController {
    ...
    override fun createReview(request: CreateReviewRequest): ResponseEntity<ReviewDetailResponse?> {
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body<ReviewDetailResponse?>(review)
    }
    ...
    override fun updateReview(id: UUID, request: UpdateReviewRequest): ResponseEntity<ReviewDetailResponse?> {
        ...
        return ResponseEntity.ok().body<ReviewDetailResponse?>(review)
    }
    ...
    override fun getAllReviewsByMovie(id: UUID, pageable: Pageable): ResponseEntity<Page<ReviewByMovieResponse?>?> {
        val reviews: Page<ReviewByMovieResponse?> = reviewService!!.getAllReviewsByMovie(id, pageable)
        return ResponseEntity.ok().body<Page<ReviewByMovieResponse?>?>(reviews)
    }

    @GetMapping("/users/{id}")
    override fun getAllReviewsByUser(id: UUID, pageable: Pageable): ResponseEntity<Page<ReviewByUserResponse?>?> {
        val reviews: Page<ReviewByUserResponse?> = reviewService!!.getAllReviewsByUser(id, pageable)
        return ResponseEntity.ok().body<Page<ReviewByUserResponse?>?>(reviews)
    }
