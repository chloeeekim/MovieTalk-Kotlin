...
@Getter
@NoArgsConstructor
class MovieDetailResponse @Builder constructor(
    ...
    @field:Schema(description = "평균 평점") private var averageRating: Double?,
    @field:Schema(description = "좋아요 수 기준 상위 리뷰 3개") private var topReviews: MutableList<ReviewByMovieResponse?>?
) {
    companion object {
        fun fromEntity(movie: Movie, topReviews: MutableList<Review?>): MovieDetailResponse? {
            return MovieDetailResponse.builder()
                ...
                .topReviews(
                    topReviews.stream()
                        .map<ReviewByMovieResponse?> { review: Review? -> ReviewByMovieResponse.fromEntity(review) }
                        .toList())
                .build()
        }
    }
